...
LocalMap::~LocalMap() {
    ...
    if (pj_normalized_) {
        proj_destroy(pj_normalized_);
        pj_normalized_ = nullptr;
    }
    if (pj_context_) {
...
int LocalMap::getBestEPSG(double lon) {
    int zone = static_cast<int>(std::round(lon / 3.0));
    // 4508 + 39 = 4547, 117E
    return 4508 + zone;
}

bool LocalMap::initProjection(int epsg_code) {
    std::string target_crs = "EPSG:" + std::to_string(epsg_code);
    PJ *P = proj_create_crs_to_crs(pj_context_, "EPSG:4326", target_crs.c_str(), NULL);
    if (P == nullptr) return false;
    pj_normalized_ = proj_normalize_for_visualization(pj_context_, P);
    proj_destroy(P);

    if (pj_normalized_) {
        epsg_code_ = epsg_code;
        return true;
    }
    return false;
}

bool LocalMap::project(double lon, double lat, double& x, double& y) {
    // initialize only once
    if (epsg_code_ == -1) {
        // 在实际产品中，建议先判断 RTK 状态是否为 FIXED，再初始化
        int best_epsg = getBestEPSG(lon);
        if (!initProjection(best_epsg)) {
            return false;
        }
        RCLCPP_INFO(get_logger(), "EPSG: %d", epsg_code_);
    }

    PJ_COORD a = proj_coord(lon, lat, 0, 0);
    PJ_COORD b = proj_trans(pj_normalized_, PJ_FWD, a);

    if (b.xy.x == HUGE_VAL || b.xy.y == HUGE_VAL) return false;
...
// // 假设你有大量的经纬度点存储在数组中
// PROJ 的 PJ_CONTEXT 是非线程安全的。如果多线程并行处理，你应该为每个线程创建一个独立的 GpsProjector 实例
// 千万不要加锁：如果在 project 函数上加 std::mutex，你会发现 96 线雷达的处理速度会瞬间掉下来。
// std::vector<double> x_coords, y_coords, z_coords; 

// // 使用 proj_trans_generic 或类似的批量接口
// // 这比在 for 循环里调用一万次 proj_trans 要快得多
// proj_trans_generic(m_pj_normalized, PJ_FWD, 
//                    x_coords.data(), sizeof(double), x_coords.size(),
//                    y_coords.data(), sizeof(double), y_coords.size(),
//                    z_coords.data(), sizeof(double), z_coords.size(),
//                    nullptr, 0, 0);
