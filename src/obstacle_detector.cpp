...
void WildTerrainSegmenter::segment(const cv::Mat& range_image, const cv::Mat& x_image, const cv::Mat& y_image, const cv::Mat& z_image, const cv::Mat& valid_mask,
    ...
    if constexpr (LOCAL_DEBUG) {
        float target_x = -0.131205;
        float target_y = 2.129133;
        float target_z = -0.386967;
    ...
    for (int r = 0; r < num_rings; ++r) {
        for (int s = 0; s < num_sectors; ++s) {
            ...
            float linearity;
            float planarity;
            estimate_plane(temp_cloud, seed_indices, cur_normal, cur_d, linearity, planarity);

            if constexpr (LOCAL_DEBUG) {
                if (r == debug_r && s == debug_s) {
                    std::cout << "Cell [" << r << ", " << s << "] Initial Plane: normal=" << cur_normal.transpose() << ", d=" << cur_d << ", seeds=" << seed_indices.size() << std::endl;
            ...
            Eigen::Vector3f pre_normal = cur_normal;
            float pre_d = cur_d;
            for (int iter = 0; iter < num_iter; ++iter) {
                ...
                for (int idx : indices) {
                    ...
                    float dist = cur_normal.dot(pt) + cur_d;
                ...
                estimate_plane(temp_cloud, seed_indices, cur_normal, cur_d, linearity, planarity);
                if (planarity == 0.0) {
                    // std::cout << "Cell [" << r << ", " << s << "] revert" << std::endl;
                    // revert and break
                    cur_normal = pre_normal;
                    cur_d = pre_d;
                    break;
                }
                else {
                    pre_normal = cur_normal;
                    pre_d = cur_d;
                }
            }

            cell_planes[r][s].normal = cur_normal;
            cell_planes[r][s].d = cur_d;
            ...
            if constexpr (LOCAL_DEBUG) {
                if (r == debug_r && s == debug_s) {
                    std::cout << "Cell [" << r << ", " << s << "] Final Plane: normal=" << cur_normal.transpose() << ", d=" << cur_d << ", seeds=" << seed_indices.size() << std::endl;
            ...
            for (int idx : indices) {
                ...
                if (dist > dist_threshold || std::abs(cur_normal.z()) < normal_z_threshold) {
...
void WildTerrainSegmenter::estimate_plane(const pcl::PointCloud<pcl::PointXYZINormal>::Ptr& cloud, const std::vector<int>& indices, 
    ...
    // The smallest eigenvalue (e0) represents the variance along plane normal. If e0 is close to 0: the plane is very thin.
    ...
    planarity = (eigenvalues(0) > 0.1) ? 0.0f : 1.0f; // if e0>0.1, should reject this plane, planarity = 0.0
